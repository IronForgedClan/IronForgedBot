...
Points from minigames & bossing: {activity_points:,}"""
    ...
    async def breakdown(
        ...
        logging.info(f'Handling /breakdown player:{player} on behalf of '
    ...
    async def ingots(
        ...
        logging.info(f'Handling /ingots player:{player} on behalf of {normalize_discord_string(interaction.user.nick)}')
        await interaction.response.defer()
    ...
    async def raffleadmin(self, interaction: discord.Interaction, subcommand: str):
        ...
        logging.info(f'Handling /raffleadmin {subcommand} on behalf of '
    ...
    async def _start_raffle(self, interaction: discord.Interaction):
        ...
        """
        try:
            self._storage_client.start_raffle(normalize_discord_string(interaction.user.nick).lower())
        except StorageError as e:
            await interaction.followup.send(
    ...
    async def _end_raffle(self, interaction: discord.Interaction):
        ...
        """
        try:
            self._storage_client.end_raffle(normalize_discord_string(interaction.user.nick).lower())
        except StorageError as e:
            await interaction.followup.send(
    ...
    async def _choose_winner(self, interaction: discord.Interaction):
        ...
        try:
            self._storage_client.delete_raffle_tickets(normalize_discord_string(interaction.user.nick).lower())
        except StorageError as e:
            await interaction.followup.send(
        ...
        for member in members:
            if member.id not in written_ids:
                ...
                new_members.append(Member(
                        id=int(member.id), runescape_name=normalize_discord_string(member.nick).lower(), ingots=0))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--upload_commands', action='store_true',
                        help='If supplied, will upload commands to discord server.')
    parser.add_argument('--tmp_dir', default='./commands_tmp', required=False,
                        help='Directory path for where to store point break downs to upload to discord.')
    parser.add_argument('--logfile', default='./ironforgedbot.log', required=False,
                        help='Path to file to write log entries to.')
    args = parser.parse_args()

    logging.basicConfig(format='%(asctime)s %(message)s', filename=args.logfile, level=logging.INFO)
