...
migrate:
	docker compose run --rm bot python -m alembic -c /usr/local/lib/python3.14/site-packages/ironforgedcore/alembic.ini upgrade head

revision:
	docker compose run --rm bot python -m alembic -c /usr/local/lib/python3.14/site-packages/ironforgedcore/alembic.ini revision --autogenerate -m "$(DESC)"

downgrade:
	docker compose run --rm bot python -m alembic -c /usr/local/lib/python3.14/site-packages/ironforgedcore/alembic.ini downgrade -1
