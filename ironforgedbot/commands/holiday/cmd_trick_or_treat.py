...
class TrickOrTreatHandler:
    def __init__(self):
        self.weights = [1 / item.weight for item in TrickOrTreat]
    ...
    def _build_no_ingots_error_response() -> discord.Embed:
    ...
    async def random_result(self, interaction: discord.Interaction):
        ...
        match action:
            ...
            case TrickOrTreat.REMOVE_ALL_INGOTS_TRICK:
                return await self.result_remove_all_ingots(interaction)
    ...
    async def result_remove_all_ingots(self, interaction: discord.Interaction):
        member = await STORAGE.read_member(
