...
def log_command_execution(logger: Optional[logging.Logger] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(interaction: discord.Interaction, *args, **kwargs):
            ...
            actual_logger = logger if logger is not None else logging.getLogger(func.__module__)
            ...
            try:
                ...
                actual_logger.info(f"Command {func.__name__} completed successfully in {elapsed:.2f}s")
                return result
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                elapsed = time.time() - start_time
                actual_logger.info(f"Task {func.__name__} completed successfully in {elapsed:.2f}s")
                return result
            except Exception as e:
                elapsed = time.time() - start_time
                actual_logger.error(
...
def log_api_call(service_name: str, logger: Optional[logging.Logger] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            actual_logger = logger if logger is not None else logging.getLogger(func.__module__)
            start_time = time.time()
            actual_logger.debug(f"API call to {service_name} via {func.__name__} started")
