    ...
    async def _execute(
        ...
        if roles_removed:
            try:
                ...
                return f":warning: {member.mention} ({member.display_name}) is no longer a **member**, but the bot lacks permission to manage their roles."

        db_member = await service.get_member_by_discord_id(member.id)
        if not db_member:
            return (
                f":warning: {member.mention} ({member.display_name}) is no longer a **member**, but "
                "cannot be found in the database."
            )

        await service.disable_member(db_member.id)

        return f":wave: {member.mention} ({member.display_name}) is no longer a **member**."
