...
class TestRemoveBlacklistedRoleHandlerShouldHandle(unittest.TestCase):
    def test_should_handle_true_when_blacklisted_role_removed(self):
        """should_handle returns True when Blacklisted role is removed."""
        handler = RemoveBlacklistedRoleHandler()

        before = create_test_member("TestUser", [ROLE.MEMBER, BLACKLISTED_ROLE_NAME])
    ...
    def test_should_handle_false_when_blacklisted_role_not_removed(self):
        """should_handle returns False when Blacklisted role was not removed."""
        handler = RemoveBlacklistedRoleHandler()

        before = create_test_member("TestUser", [ROLE.MEMBER, BLACKLISTED_ROLE_NAME])
        after = create_test_member("TestUser", [ROLE.MEMBER, BLACKLISTED_ROLE_NAME])
        after.id = before.id
        report_channel = Mock(spec=discord.TextChannel)
...
class TestRemoveBlacklistedRoleHandlerExecute(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.handler = RemoveBlacklistedRoleHandler()
        self.mock_session = AsyncMock()
        self.mock_service = AsyncMock()

    def _create_context(self, before_roles=None, after_roles=None):
        before_roles = before_roles or [ROLE.MEMBER, BLACKLISTED_ROLE_NAME]
    ...
    async def test_execute_updates_blacklisted_flag(self):
        """Updates is_blacklisted flag to False in database."""
        ...
        self.mock_service.update_member_flags.assert_called_once_with(
            "test-id", is_blacklisted=False
        )

    async def test_execute_returns_message(self):
        """Returns appropriate message when Blacklisted role is removed."""
...
class TestRemoveBlacklistedRoleHandlerPriority(unittest.TestCase):
    def test_priority_is_20(self):
        """RemoveBlacklistedRoleHandler has priority 20."""
        handler = RemoveBlacklistedRoleHandler()
        self.assertEqual(handler.priority, 20)

    def test_name_property(self):
        """Handler name is RemoveBlacklistedRole."""
        handler = RemoveBlacklistedRoleHandler()
        self.assertEqual(handler.name, "RemoveBlacklistedRole")
