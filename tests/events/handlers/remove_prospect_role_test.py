...
class TestRemoveProspectRoleHandlerShouldHandle(unittest.TestCase):
    def test_should_handle_true_when_prospect_role_removed(self):
        """should_handle returns True when Prospect role is removed."""
        handler = RemoveProspectRoleHandler()
    ...
    def test_should_handle_false_when_prospect_role_not_removed(self):
        """should_handle returns False when Prospect role was not removed."""
        handler = RemoveProspectRoleHandler()
...
class TestRemoveProspectRoleHandlerExecute(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.handler = RemoveProspectRoleHandler()
        self.mock_session = AsyncMock()
        self.mock_service = AsyncMock()

    def _create_context(self, before_roles=None, after_roles=None):
        before_roles = before_roles or [ROLE.MEMBER, PROSPECT_ROLE_NAME]
    ...
    async def test_execute_updates_prospect_flag(self):
        """Updates is_prospect flag to False in database."""
        ...
        self.mock_service.update_member_flags.assert_called_once_with(
            "test-id", is_prospect=False
        )

    async def test_execute_returns_message(self):
        """Returns appropriate message when Prospect role is removed."""
...
class TestRemoveProspectRoleHandlerPriority(unittest.TestCase):
    def test_priority_is_20(self):
        """RemoveProspectRoleHandler has priority 20."""
        handler = RemoveProspectRoleHandler()
        self.assertEqual(handler.priority, 20)

    def test_name_property(self):
        """Handler name is RemoveProspectRole."""
        handler = RemoveProspectRoleHandler()
        self.assertEqual(handler.name, "RemoveProspectRole")
