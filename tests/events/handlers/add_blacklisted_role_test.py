...
class TestAddBlacklistedRoleHandlerShouldHandle(unittest.TestCase):
    def test_should_handle_true_when_blacklisted_role_added(self):
        """should_handle returns True when Blacklisted role is added."""
        handler = AddBlacklistedRoleHandler()
    ...
    def test_should_handle_false_when_blacklisted_role_not_added(self):
        """should_handle returns False when Blacklisted role was not added."""
        handler = AddBlacklistedRoleHandler()
...
class TestAddBlacklistedRoleHandlerExecute(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.handler = AddBlacklistedRoleHandler()
        self.mock_session = AsyncMock()
        self.mock_service = AsyncMock()

    def _create_context(self, before_roles=None, after_roles=None):
        before_roles = before_roles or [ROLE.MEMBER]
        after_roles = after_roles or [ROLE.MEMBER, BLACKLISTED_ROLE_NAME]
    ...
    async def test_execute_updates_blacklisted_flag(self):
        """Updates is_blacklisted flag to True in database."""
        ...
        self.mock_service.update_member_flags.assert_called_once_with(
            "test-id", is_blacklisted=True
        )

    async def test_execute_returns_message(self):
        """Returns appropriate message when Blacklisted role is added."""
...
class TestAddBlacklistedRoleHandlerPriority(unittest.TestCase):
    def test_priority_is_20(self):
        """AddBlacklistedRoleHandler has priority 20."""
        handler = AddBlacklistedRoleHandler()
        self.assertEqual(handler.priority, 20)

    def test_name_property(self):
        """Handler name is AddBlacklistedRole."""
        handler = AddBlacklistedRoleHandler()
        self.assertEqual(handler.name, "AddBlacklistedRole")
