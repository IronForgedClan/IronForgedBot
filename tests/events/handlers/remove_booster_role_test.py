...
class TestRemoveBoosterRoleHandlerShouldHandle(unittest.TestCase):
    def test_should_handle_true_when_booster_role_removed(self):
        """should_handle returns True when Server Booster role is removed."""
        handler = RemoveBoosterRoleHandler()
    ...
    def test_should_handle_false_when_booster_role_not_removed(self):
        """should_handle returns False when Server Booster role was not removed."""
        handler = RemoveBoosterRoleHandler()
...
class TestRemoveBoosterRoleHandlerExecute(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.handler = RemoveBoosterRoleHandler()
        self.mock_session = AsyncMock()
        self.mock_service = AsyncMock()

    def _create_context(self, before_roles=None, after_roles=None):
        before_roles = before_roles or [ROLE.MEMBER, BOOSTER_ROLE_NAME]
    ...
    async def test_execute_updates_booster_flag(self):
        """Updates is_booster flag to False in database."""
        ...
        self.mock_service.update_member_flags.assert_called_once_with(
            "test-id", is_booster=False
        )

    async def test_execute_returns_message(self):
        """Returns appropriate message when Server Booster role is removed."""
...
class TestRemoveBoosterRoleHandlerPriority(unittest.TestCase):
    def test_priority_is_20(self):
        """RemoveBoosterRoleHandler has priority 20."""
        handler = RemoveBoosterRoleHandler()
        self.assertEqual(handler.priority, 20)

    def test_name_property(self):
        """Handler name is RemoveBoosterRole."""
        handler = RemoveBoosterRoleHandler()
        self.assertEqual(handler.name, "RemoveBoosterRole")
