...
class TestIronForgedAutomations(unittest.IsolatedAsyncioTestCase):
    ...
    def setUpClass(cls):
        ...
        warnings.filterwarnings("ignore", message="coroutine.*AsyncMockMixin.*was never awaited", category=RuntimeWarning)
        warnings.filterwarnings("ignore", message="coroutine.*IronForgedAutomations.*was never awaited", category=RuntimeWarning)
        warnings.filterwarnings("ignore", message="coroutine.*cleanup.*was never awaited", category=RuntimeWarning)
        warnings.filterwarnings("ignore", message="coroutine.*_safe_job_wrapper.*was never awaited", category=RuntimeWarning)
    ...
    async def test_track_job_creates_and_tracks_task(self):
        ...
        with patch('ironforgedbot.automations.asyncio.create_task', return_value=mock_task):
