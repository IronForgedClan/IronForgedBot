...
class TestScoreCache(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.test_score_data = create_test_score_breakdown(skills_count=3, activities_count=2)
    ...
    async def test_clean_expired_entries(self, mock_time, mock_getsizeof):
        ...
        self.assertIsNotNone(result)
        self.assertIn("Deleted **1.01 KB**", result)  # Adjusted to match actual calculation
    ...
    async def test_clean_mixed_expired_and_valid_entries(self, mock_time):
        ...
        with patch("ironforgedbot.cache.score_cache.deep_getsizeof", side_effect=[2048, 1024]):
            result = await self.cache.clean()
    ...
    async def test_concurrent_access_with_locks(self):
        ...
        async def setter_task():
            for i in range(5):
                await self.cache.set(f"player_{i}", create_test_score_breakdown(skills_count=i+1))
    ...
    async def test_cache_size_calculation_edge_cases(self, mock_time):
        ...
        with patch("ironforgedbot.cache.score_cache.deep_getsizeof", side_effect=[0, 0]):
            result = await self.cache.clean()
            self.assertIsNotNone(result)
    ...
    async def test_cleanup_logging(self, mock_time, mock_logging):
        ...
        with patch("ironforgedbot.cache.score_cache.deep_getsizeof", side_effect=[1024, 512]):
