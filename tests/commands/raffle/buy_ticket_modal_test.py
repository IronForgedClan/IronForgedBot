...
class TestBuyTicketModal(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_modal_submission_success(self, mock_state, mock_raffle_service_class, mock_db, mock_build_embed, mock_find_emoji, mock_normalize):
    ...
    @patch("ironforgedbot.commands.raffle.buy_ticket_modal.normalize_discord_string")
    async def test_modal_submission_invalid_quantity(self, mock_normalize, mock_send_error):
        mock_normalize.return_value = "TestUser"
    ...
    @patch("ironforgedbot.commands.raffle.buy_ticket_modal.find_emoji")
    @patch("ironforgedbot.commands.raffle.buy_ticket_modal.build_response_embed")
    async def test_modal_submission_negative_quantity(self, mock_build_embed, mock_find_emoji, mock_normalize):
        mock_normalize.return_value = "TestUser"
    ...
    @patch("ironforgedbot.commands.raffle.buy_ticket_modal.RaffleService")
    @patch("ironforgedbot.commands.raffle.buy_ticket_modal.STATE")
    async def test_modal_submission_insufficient_funds(self, mock_state, mock_raffle_service_class, mock_db, mock_normalize, mock_send_error):
        mock_state.state = {"raffle_price": 5000}
        mock_normalize.return_value = "TestUser"
        ...
        mock_send_error.assert_called_once_with(self.mock_interaction, "Insufficient ingots")