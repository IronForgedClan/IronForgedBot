...
class ClientTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.client = DiscordClient(
            intents=self.mock_intents, upload=True, guild=self.mock_guild
        )
    ...
    @patch("ironforgedbot.client.logger")
    async def test_graceful_shutdown_completes_successfully(
        self,
        mock_logger,
        mock_current_task,
        mock_all_tasks,
    ...
    async def test_setup_hook_loads_state_and_syncs_commands(
        self, mock_populate_emoji, mock_state
    ):
    ...
    @patch("ironforgedbot.client.STATE")
    @patch("ironforgedbot.client.populate_emoji_cache")
    async def test_setup_hook_skips_sync_when_upload_false(
        self, mock_populate_emoji, mock_state
    ):
        mock_state.load_state = AsyncMock()
        mock_tree = Mock()
    ...
    async def test_on_ready_sets_presence_and_automations(
        self, mock_automations_class, mock_config
    ):
        ...
        with patch.object(
            DiscordClient, "user", new_callable=PropertyMock
        ) as mock_user_prop:
    ...
    async def test_on_ready_exits_when_no_user(self, mock_logger, mock_exit):
        with patch.object(
            ...
            mock_logger.critical.assert_called_once_with(
                "Error logging into discord server"
            )
    ...
    @patch("ironforgedbot.client.get_text_channel")
    @patch("ironforgedbot.client.logger")
    async def test_on_member_update_handles_missing_channel(
        self, mock_logger, mock_get_channel, mock_config
    ):
        mock_config.AUTOMATION_CHANNEL_ID = 555666777
