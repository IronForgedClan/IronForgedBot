...
def setup_database_service_mocks(mock_db, mock_service_class, mock_service_instance=None):
...
def assert_embed_structure(test_case, interaction_mock, expected_title=None, 
                          expected_description=None, expected_field_count=None, 
                          expected_color=None, expected_fields=None):
...
def assert_followup_called_with_embed(test_case, interaction_mock, embed_title=None, 
                                    has_file=False, call_count=1):
...
def create_test_db_member(nickname="TestUser", discord_id=None, rank="Iron", 
                         ingots=1000, active=True, joined_date=None, **kwargs):
...
def create_wom_response_mock(status=200, skills_data=None, activities_data=None, 
                           player_data=None, name_changes_data=None):
...
def setup_time_mocks(mock_datetime, mock_time, fixed_datetime=None, duration_seconds=5.0):
...
def create_test_score_data(skills_count=2, activities_count=2, total_points=1500):
    ...
    breakdown.bosses = activities[2:] if len(activities) > 2 else activities[-1:] if activities else []
    
    return breakdown


def create_test_member_with_scores(nickname="TestUser", discord_id=None, rank="Iron", 
                                 total_points=1500, **kwargs):
