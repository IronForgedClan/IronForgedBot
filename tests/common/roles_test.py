...
class TestRoles(unittest.TestCase):
    ...
    def create_mock_role(self, name):
        mock_role = Mock()
        mock_role.name = name
        return mock_role
    ...
    def test_check_member_has_role_exact_match(self):
        self.mock_member.roles = [self.create_mock_role("Member")]
    ...
    def test_check_member_has_role_or_higher_success(self):
        self.mock_member.roles = [self.create_mock_role("Leadership")]
    ...
    def test_check_member_has_role_or_lower_fail(self):
        self.mock_member.roles = [self.create_mock_role("Staff")]
    ...
    def test_member_has_any_roles_success(self):
        self.mock_member.roles = [self.create_mock_role("Member"), self.create_mock_role("Staff")]
        result = member_has_any_roles(self.mock_member, [ROLE.MEMBER, ROLE.LEADERSHIP])
        self.assertTrue(result)

    def test_member_has_any_roles_fail(self):
        self.mock_member.roles = [self.create_mock_role("Guest")]
        result = member_has_any_roles(self.mock_member, [ROLE.MEMBER, ROLE.LEADERSHIP])
        self.assertFalse(result)
    ...
    def test_is_member_banned_true(self):
        self.mock_member.roles = [self.create_mock_role("Slag")]
    ...
    def test_check_member_has_role_multiple_roles(self):
        self.mock_member.roles = [
            self.create_mock_role("Guest"),
            self.create_mock_role("Member"),
            self.create_mock_role("Staff")
        ]
