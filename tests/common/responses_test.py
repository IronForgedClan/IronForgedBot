...
class TestResponses(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_send_prospect_response_success(self, mock_member_service_class, mock_db, mock_build_embed, mock_text_bold, mock_find_emoji):
        ...
        self.assertIn("**TestPlayer**", call_args[0][1])  # description contains member name
    ...
    @patch("ironforgedbot.common.responses.db")
    @patch("ironforgedbot.common.responses.MemberService")
    async def test_send_prospect_response_member_not_found(self, mock_member_service_class, mock_db, mock_send_error):
        mock_session = AsyncMock()
        mock_db.get_session.return_value.__aenter__.return_value = mock_session
    ...
    @patch("ironforgedbot.common.responses.text_bold")
    @patch("ironforgedbot.common.responses.build_response_embed")
    async def test_send_member_no_hiscore_values(self, mock_build_embed, mock_text_bold, mock_get_rank_color, mock_get_rank, mock_find_emoji):
        ...
        self.assertIn("Unable to calculate an accurate score", call_args[0][1])  # description
    ...
    @patch("ironforgedbot.common.responses.text_bold")
    @patch("ironforgedbot.common.responses.build_response_embed")
    async def test_send_not_clan_member_with_points(self, mock_build_embed, mock_text_bold):
        mock_text_bold.side_effect = lambda x: f"**{x}**"
        mock_embed = Mock()
        mock_build_embed.return_value = mock_embed
        ...
        call_args = mock_build_embed.call_args
        self.assertEqual(call_args[0][0], "⚪ TestPlayer")  # title
        self.assertIn("This player is not a member of the clan", call_args[0][1])  # description
    ...
    @patch("ironforgedbot.common.responses.build_response_embed")
    async def test_send_not_clan_member_no_points(self, mock_build_embed, mock_text_sub):
        ...
        mock_build_embed.assert_called_once()
        call_args = mock_build_embed.call_args
        self.assertEqual(call_args[0][0], ":grey_question: TestPlayer")  # title with question mark
        self.assertIn("Unable to calculate an accurate score", call_args[0][1])  # description
    ...
    @patch("ironforgedbot.common.responses.db")
    @patch("ironforgedbot.common.responses.MemberService")
    async def test_send_prospect_response_days_calculation(self, mock_member_service_class, mock_db, mock_build_embed, mock_text_bold, mock_find_emoji):
        mock_find_emoji.return_value = "🔸"
        mock_text_bold.side_effect = lambda x: f"**{x}**"
