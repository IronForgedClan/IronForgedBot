...
class AddMemberRoleTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_creates_new_member_successfully(self, mock_db, mock_get_rank, mock_time):
        ...
        with patch("ironforgedbot.effects.add_member_role.MemberService", return_value=mock_service):
            await add_member_role(self.mock_report_channel, self.discord_member)

        mock_service.create_member.assert_called_once_with(123456789, "TestUser", RANK.IRON)
    ...
    @patch("ironforgedbot.effects.add_member_role.db")
    async def test_converts_god_alignment_ranks_to_god(self, mock_db, mock_god_alignment, mock_get_rank, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
        mock_get_rank.return_value = RANK.GOD_SARADOMIN
        mock_god_alignment.list.return_value = [RANK.GOD_SARADOMIN, RANK.GOD_ZAMORAK, RANK.GOD_GUTHIX]
        mock_session = AsyncMock()
        mock_db.get_session.return_value.__aenter__.return_value = mock_session
        mock_service = Mock()
        mock_member = Mock()
        ...
        mock_service.create_member.assert_called_once_with(123456789, "TestUser", RANK.GOD)

    @patch("ironforgedbot.effects.add_member_role.time")
    @patch("ironforgedbot.effects.add_member_role.get_rank_from_member")
    @patch("ironforgedbot.effects.add_member_role.db")
    async def test_defaults_to_iron_rank_when_none(self, mock_db, mock_get_rank, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
    ...
    @patch("ironforgedbot.effects.add_member_role.get_rank_from_member")
    @patch("ironforgedbot.effects.add_member_role.db")
    async def test_reactivates_inactive_member_on_discord_id_violation(self, mock_db, mock_get_rank, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
        mock_get_rank.return_value = RANK.IRON
        mock_session = AsyncMock()
        mock_db.get_session.return_value.__aenter__.return_value = mock_session
        ...
        mock_reactivate_response.previous_join_date = datetime(2023, 1, 1, tzinfo=timezone.utc)
        mock_reactivate_response.approximate_leave_date = datetime(2023, 6, 1, tzinfo=timezone.utc)
        ...
        mock_service.get_member_by_discord_id = AsyncMock(return_value=mock_existing_member)
        mock_service.reactivate_member = AsyncMock(return_value=mock_reactivate_response)

        with patch("ironforgedbot.effects.add_member_role.MemberService", return_value=mock_service), \
             patch("ironforgedbot.effects.add_member_role.find_emoji") as mock_emoji, \
             patch("ironforgedbot.effects.add_member_role.build_response_embed") as mock_embed:
    ...
    @patch("ironforgedbot.effects.add_member_role.db")
    async def test_handles_nickname_conflict_on_reactivation(self, mock_db, mock_rollback, mock_get_rank, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
        mock_get_rank.return_value = RANK.IRON
        mock_session = AsyncMock()
        mock_db.get_session.return_value.__aenter__.return_value = mock_session
        ...
        mock_service.create_member = AsyncMock(side_effect=UniqueDiscordIdVolation())
        mock_service.get_member_by_discord_id = AsyncMock(return_value=mock_existing_member)
        mock_service.reactivate_member = AsyncMock(side_effect=UniqueNicknameViolation())
        mock_service.get_member_by_nickname = AsyncMock(return_value=mock_conflicting_db_member)
        self.mock_guild.get_member_named.return_value = mock_conflicting_member

        with patch("ironforgedbot.effects.add_member_role.MemberService", return_value=mock_service):
            await add_member_role(self.mock_report_channel, self.discord_member)

        mock_rollback.assert_called_once_with(self.mock_report_channel, self.discord_member)
        self.mock_report_channel.send.assert_called_once()
        call_args = self.mock_report_channel.send.call_args[0][0]
    ...
    @patch("ironforgedbot.effects.add_member_role.db")
    async def test_handles_generic_exception(self, mock_db, mock_logger, mock_rollback, mock_get_rank, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
        mock_get_rank.return_value = RANK.IRON
        mock_session = AsyncMock()
        mock_db.get_session.return_value.__aenter__.return_value = mock_session
    ...
    @patch("ironforgedbot.effects.add_member_role._rollback")
    @patch("ironforgedbot.effects.add_member_role.db")
    async def test_handles_none_member_result(self, mock_db, mock_rollback, mock_get_rank, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
        mock_get_rank.return_value = RANK.IRON
        mock_session = AsyncMock()
        mock_db.get_session.return_value.__aenter__.return_value = mock_session
    ...
    @patch("ironforgedbot.effects.add_member_role.get_rank_from_member")
    @patch("ironforgedbot.effects.add_member_role.db")
    async def test_handles_active_member_on_unique_violation(self, mock_db, mock_get_rank, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
        mock_get_rank.return_value = RANK.IRON
        mock_session = AsyncMock()
        mock_db.get_session.return_value.__aenter__.return_value = mock_session
