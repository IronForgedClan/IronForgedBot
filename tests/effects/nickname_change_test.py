...
class NicknameChangeTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_updates_nickname_successfully(self, mock_db, mock_normalize, mock_time):
        ...
        with patch("ironforgedbot.effects.nickname_change.MemberService", return_value=mock_service):
            await nickname_change(self.mock_report_channel, self.before_member, self.after_member)
    ...
    @patch("ironforgedbot.effects.nickname_change.db")
    async def test_handles_member_not_found_in_database(self, mock_db, mock_rollback, mock_normalize, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
        mock_normalize.return_value = "NewNick"
        ...
        mock_rollback.assert_called_once_with(self.mock_report_channel, self.after_member, "OldNick")
        self.mock_report_channel.send.assert_called_once()
        call_args = self.mock_report_channel.send.call_args[0][0]
    ...
    @patch("ironforgedbot.effects.nickname_change._rollback")
    @patch("ironforgedbot.effects.nickname_change.db")
    async def test_handles_member_not_found_rollback_failure(self, mock_db, mock_rollback, mock_normalize, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
        mock_normalize.return_value = "NewNick"
    ...
    @patch("ironforgedbot.effects.nickname_change.normalize_discord_string")
    @patch("ironforgedbot.effects.nickname_change.db")
    async def test_ignores_when_nickname_unchanged(self, mock_db, mock_normalize, mock_time):
    ...
    @patch("ironforgedbot.effects.nickname_change._rollback")
    @patch("ironforgedbot.effects.nickname_change.db")
    async def test_handles_nickname_conflict_with_discord_member(self, mock_db, mock_rollback, mock_normalize, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
    ...
    @patch("ironforgedbot.effects.nickname_change._rollback")
    @patch("ironforgedbot.effects.nickname_change.db")
    async def test_handles_nickname_conflict_without_discord_member(self, mock_db, mock_rollback, mock_normalize, mock_time):
        mock_time.perf_counter.side_effect = [0.0, 5.0]
        mock_normalize.return_value = "ConflictName"
        mock_rollback.return_value = True
        mock_session = AsyncMock()
    ...
    @patch("ironforgedbot.effects.nickname_change._rollback")
    @patch("ironforgedbot.effects.nickname_change.db")
    async def test_handles_conflict_with_missing_conflicting_member(self, mock_db, mock_rollback, mock_normalize, mock_time):
        mock_normalize.return_value = "ConflictName"
        mock_rollback.return_value = True
        mock_session = AsyncMock()
        mock_db.get_session.return_value.__aenter__.return_value = mock_session
    ...
    @patch("ironforgedbot.effects.nickname_change._rollback")
    @patch("ironforgedbot.effects.nickname_change.db")
    async def test_handles_conflict_rollback_failure(self, mock_db, mock_rollback, mock_normalize, mock_time):
        mock_normalize.return_value = "ConflictName"
        mock_rollback.return_value = False
        mock_session = AsyncMock()
        mock_db.get_session.return_value.__aenter__.return_value = mock_session
